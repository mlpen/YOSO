...
for attn_type in attn_config:
    ...
    for log_seq_len in reversed(range(7, 13)):
        ...
        batch_size, per_inst_time_avg, per_inst_time_std, memory_per_inst = get_time_memory(config, seq_len, True)
    ...
    with open(f"profile_seq_len_max_batch/{attn_type}.json", "w") as f:
